 ...
 */
class HrParagraphsRWReportsTest extends ExistingSiteBase {
   ...
   */
  public function testExternalReportsTabOnOperation() {
    ...
    $group->set('field_reliefweb_documents', 'https://reliefweb.int/updates?advanced-search=%28C13%29_%28OT271%29_%28T4587%29_%28F5%29');
    $group->set('field_documents_page', 'https://www.example.com');
    ...
    $url = Url::fromRoute('hr_paragraphs.operation.reports', [
      'group' => $group->id(),
    ]);

    $this->drupalGet($url);
    $this->assertSession()->pageTextNotContains('Reports');
  }

  /**
   * Test RW data on group.
   */
  public function testReportsTabOnOperation() {
    $group_title = 'Operation X';
    ...
    $this->drupalGet($url);
    $this->assertSession()->elementTextEquals('css', 'h1.cd-page-title', 'Reports');
  }
  ...
  protected function renderGroupTab($group) {
    ...
    $build = $paragraph_controller->getReports($group, $request);
    $output = \Drupal::service('renderer')->renderRoot($build);
  ...
  protected function renderGroupTabRedirect($group) {
    ...
    $output =  $paragraph_controller->getReports($group, $request);
