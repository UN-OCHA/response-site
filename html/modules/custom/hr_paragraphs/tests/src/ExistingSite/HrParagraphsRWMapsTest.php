 ...
 */
class HrParagraphsRWMapsTest extends ExistingSiteBase {
   ...
   */
  public function testExternalMapsTabOnOperation() {
    ...
    $group->set('field_maps_infographics_link', 'https://reliefweb.int/updates?advanced-search=%28C13%29_%28OT271%29_%28T4587%29_%28F5%29');
    $group->set('field_infographics', 'https://www.example.com');
    ...
    $url = Url::fromRoute('hr_paragraphs.operation.maps', [
      'group' => $group->id(),
    ]);

    $this->drupalGet($url);
    $this->assertSession()->pageTextNotContains('Maps / Infographics');
  }

  /**
   * Test RW data on group.
   */
  public function testMapsTabOnOperation() {
    $group_title = 'Operation X';
    ...
    $this->drupalGet($url);
    $this->assertSession()->elementTextEquals('css', 'h1.cd-page-title', 'Maps / Infographics');
  }
  ...
  protected function renderGroupTab($group) {
    ...
    $build = $paragraph_controller->getInfographics($group, $request);
    $output = \Drupal::service('renderer')->renderRoot($build);
  ...
  protected function renderGroupTabRedirect($group) {
    ...
    $output =  $paragraph_controller->getInfographics($group, $request);
